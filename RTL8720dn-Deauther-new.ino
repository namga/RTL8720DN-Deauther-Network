...
char *ssid = "NamGa NetWork";
char *pass = "0968387974";
  ...
  if(millis() - server_handle >= 1000)
  {
    ...
    if (client)
    {
      ...
      else if (request.indexOf("/buttonA") != -1)
      {
        ...
        number_network_selected = 0;
        scan_n();
        digitalWrite(LED_G,LOW);
        Serial.println("Request button A re-scan network is start!!");
      }
      else if (request.indexOf("/buttonB") != -1)
      {
        if (number_network_selected != 0)
        {
          startDeauth();
          Serial.println("Request button B deauther is start!!");
        }
      } 
      else if (request.indexOf("/buttonC") != -1)
      {
        number_network_selected = 0;
        stop_n();
        Serial.println("Request button C cancle is start!!");
      }
...
void sendHomePage(WiFiClient& client)
{
  ...
  client.println("h2 { color: #ff0000; text-align: center; font-size: 16px }");
  ...
  client.println(".btn:active { transform: translateY(4px); }");
  ...
  client.println("<h1>NamGa WiFi Deauther for 5ghz/2.4ghz</h1>");
  client.println("<h2>Just using for study and research</h2>");
